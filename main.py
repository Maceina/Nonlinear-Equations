...
# 3 - Niutono(liestiniu) 
# 5 - Skenavimo su mazejanciu zingsniusass


# Funkcijos:
# f(x) = 2.19x^4 - 5.17x^3 - 7.17x^2 + 15.14x + 1.21
# g(x) = e^(-(x/2)^2)*sin(2x); -6 <= x <= 6
...
def f(x):
    return 2.19*x**4 - 5.17*x**3 - 7.17*x**2 + 15.14*x + 1.21


def df(x):
    return 8.76*x**3 - 15.51*x**2 - 14.34*x + 15.14


def g(x):
    return np.e**-(x/2)**2 * np.sin(2*x)


def dg(x):
    return np.e**((x**2)/4) * (x*np.sin(2*x) + 4*np.cos(2*x))


def v(x):
    return 50 * np.e**(6 * x) + (4.9 / x) * (np.e** (-6 * x) - 1) - 14
